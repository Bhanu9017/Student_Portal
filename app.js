...
// Subject config — teacher (Dr. Anita Bose) can only edit Data Structures
var SUBJECTS = [
  { key: 'ds',   name: 'Data Structures',   editable: true  },
  { key: 'os',   name: 'Operating Systems', editable: false },
  { key: 'dbms', name: 'DBMS',              editable: false },
  { key: 'cn',   name: 'Computer Networks', editable: false },
  { key: 'math', name: 'Mathematics-III',   editable: false }
];

// Default marks per student per subject { mid, end }
var DEFAULT_MARKS = {
  STU001: {
    ds:   { mid: 42, end: 78 },
    os:   { mid: 38, end: 70 },
...
// Teacher can only edit their subject (ds). Overall score uses ds only for backward compat.
var TEACHER_SUBJECT_KEY = 'ds';
...
function lsLoadMarks() {
  try {
    var raw = localStorage.getItem('ep-teacher-marks-v2');
...
function lsSaveMarks(marks) {
  localStorage.setItem('ep-teacher-marks-v2', JSON.stringify(marks));
}
...
function getTeacherMarks() {
  ...
  Object.keys(STUDENT_DATA).forEach(function(roll) {
    ...
    SUBJECTS.forEach(function(subj) {
      var def = DEFAULT_MARKS[roll][subj.key];
      var sv = saved[roll] && saved[roll][subj.key];
      merged[roll][subj.key] = {
        mid: Number(sv && sv.mid != null ? sv.mid : def.mid),
        end: Number(sv && sv.end != null ? sv.end : def.end)
      };
    });
  });
  return merged;
...
function syncStudentAcademicData() {
  ...
  Object.keys(STUDENT_DATA).forEach(function(roll) {
    ...
    var totalScore = 0;
    SUBJECTS.forEach(function(subj) {
      totalScore += marks[roll][subj.key].mid + marks[roll][subj.key].end;
    });
    var avg = Math.round(totalScore / SUBJECTS.length / 150 * 100);
...
function renderTeacherStudentList() {
  ...
  if (!body) return;
  // Only show CSE students in teacher's dashboard
  var cseRolls = Object.keys(STUDENT_DATA).filter(function(roll) {
    var branch = STUDENT_DATA[roll].branch.toLowerCase();
    return branch.indexOf('computer') !== -1;
  });
  body.innerHTML = cseRolls.map(function(roll) {
    var s = STUDENT_DATA[roll];
...
function renderTeacherPerformance() {
  ...
  if (!body) return;
  var cseRolls = Object.keys(STUDENT_DATA).filter(function(roll) {
    return STUDENT_DATA[roll].branch.toLowerCase().indexOf('computer') !== -1;
  });
  body.innerHTML = cseRolls.map(function(roll) {
    var s = STUDENT_DATA[roll];
...
function renderTeacherMarks() {
  ...
  if (!body) return;
  var marks = getTeacherMarks();
  // Teacher can only edit their subject (Data Structures)
  var teacherSubj = SUBJECTS.find(function(s) { return s.editable; }) || SUBJECTS[0];
  // Only show CSE students
  var cseRolls = Object.keys(STUDENT_DATA).filter(function(roll) {
    return STUDENT_DATA[roll].branch.toLowerCase().indexOf('computer') !== -1 ||
           STUDENT_DATA[roll].branch === 'CSE';
  });
  body.innerHTML = cseRolls.map(function(roll) {
    var s = STUDENT_DATA[roll];
    var m = marks[roll][teacherSubj.key];
    var total = m.mid + m.end;
    var pct = Math.round(total / 150 * 100);
    var grade = getGrade(pct);
    return '<tr>' +
      '<td>' + roll + '</td>' +
      '<td><strong>' + s.name + '</strong></td>' +
      '<td><input class="marks-input" type="number" min="0" max="50" value="' + m.mid + '" onchange="saveStudentMarks(\'' + roll + '\', \'' + teacherSubj.key + '\', \'mid\', this.value)" /></td>' +
      '<td><input class="marks-input" type="number" min="0" max="100" value="' + m.end + '" onchange="saveStudentMarks(\'' + roll + '\', \'' + teacherSubj.key + '\', \'end\', this.value)" /></td>' +
...
function buildStudentMarksRows(roll) {
  ...
  return SUBJECTS.map(function(subj) {
    var m = marks[roll][subj.key];
    var total = m.mid + m.end;
    var pct = Math.round(total / 150 * 100);
    var grade = getGrade(pct);
    var fillClass = getScoreClass(pct);
    var demoTag = subj.editable ? '' : '<span style="font-size:.6rem;color:var(--text-muted);margin-left:.35rem;opacity:.7;">(demo)</span>';
    return '<tr>' +
      '<td>' + subj.name + demoTag + '</td>' +
      '<td>' + m.mid + '</td>' +
      '<td>' + m.end + '</td>' +
      '<td><strong>' + total + '</strong>/150</td>' +
      '<td>' + pct + '%</td>' +
...
function buildStudentDashboardMarksRows(roll) {
  ...
  return SUBJECTS.map(function(subj) {
    ...
    var pct = Math.round(total / 150 * 100);
    var grade = getGrade(pct);
    var demoTag = subj.editable ? '' : '<span style="font-size:.55rem;color:var(--text-muted);margin-left:.3rem;opacity:.7;">(demo)</span>';
    return '<tr><td>' + subj.name + demoTag + '</td><td>' + total + '/150</td><td><span class="grade ' + getGradeClass(grade) + '">' + grade + '</span></td></tr>';
  }).join('');
}
...
function renderStudentParentViews() {
  ...
  var dsMarks = marks[TEACHER_SUBJECT_KEY]; // Data Structures — teacher's editable subject
  var score = parseInt(s.score, 10);
  var att = parseInt(s.attendance, 10);
  ...
  setText('student-marks-mid', dsMarks.mid);
  setText('student-marks-end', dsMarks.end);
  ...
  setText('parent-marks-mid', dsMarks.mid);
  setText('parent-marks-end', dsMarks.end);
...
function saveStudentMarks(roll, subjKey, field, value) {
  var marks = getTeacherMarks();
  var max = field === 'mid' ? 50 : 100;
  var next = Math.max(0, Math.min(max, Number(value) || 0));
  marks[roll][subjKey][field] = next;
  lsSaveMarks(marks);
  syncStudentAcademicData();
